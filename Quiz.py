...
class Quiz():
    def __init__(self,root,sh):
        ...
        buttonNext = tkinter.Button(root,text = 'Next', padx = 30, command = lambda: Quiz.change_word(self,sh,labelWordPl,labelresult,entry))
        buttonBack = tkinter.Button(root,text = 'Back', padx = 30)
        buttonCheck = tkinter.Button(root,text = 'Check', padx = 10, command = lambda: Quiz.read_Eword_typed(self,entry,labelresult))
    ...
    def read_Eword_typed(self,entry,label):
        Ewordtyped = entry.get()
        if self.Eword.value == Ewordtyped:
            label.config(text = 'Git!')
        else:
            label.config(text = 'BUU! :c')
